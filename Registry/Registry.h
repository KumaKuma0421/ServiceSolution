...
public:
	...
	//! @retval TRUE(成功)
	//! @retval FALSE(失敗)
	BOOL Create (HKEY hRootKey, LPCTSTR lpctszSubKey, BOOL bVoratile = FALSE);
	...
	//! @param[in]  samDesired 開くキーに対する必要なアクセス権を指定するマスク
	//!   - KEY_READ
	//!   - KEY_WRITE
	//!   - KEY_EXECUTE
	//!   - KEY_ALL_ACCESS
	...
	//! @retval TRUE(成功)
	//! @retval FALSE(失敗)
	BOOL Open (HKEY hRootKey, LPCTSTR lpctszSubKey, REGSAM samDesired = KEY_ALL_ACCESS);
